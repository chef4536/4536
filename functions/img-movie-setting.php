...
<?php }
if(get_option('thumbnail_generate_4536')===false) update_option('thumbnail_generate_4536', 1);
...
/////////////////////////////
add_filter('the_content', function($the_content) {
    if(kaereba_convert()=='amp' && !is_amp()) return $the_content;
    ...
    if(preg_match_all($search, $the_content, $match)) {
        foreach($match[0] as $kaereba_image) {
            ...
            if(preg_match('/<a.+?>/i', $kaereba_image, $link)) $link = $link[0];
            if(preg_match('/<img.+?src=["\']([^"\']+?)["\'].+?\/?>/i', $kaereba_image, $image)) $src = $image[1];
            if(preg_match('/<\/a>/i', $kaereba_image, $e_link)) $e_link = $e_link[0];
            if(preg_match('/<\/div>/i', $kaereba_image, $e)) $e = $e[0];
            $class = get_thumbnail_class_4536($src);
...
function get_image_width_and_height_4536($image_url) {
    ...
    if(strpos($image_url, $wp_content_url) === false) return $res;
...
function auto_post_thumbnail_image_4536() {
  ...
  if ( isset($post) && isset($post->ID) ) {
    ...
    if (empty($matches[0])) {
      ...
      if (!empty($match[1])) {
        $matches=array(); $matches[0]=$matches[1]=array('http://img.youtube.com/vi/'.$match[1].'/maxresdefault.jpg');
      }
    }
...
if(get_post_first_image()=='original') add_action( 'save_post', 'save_original_thumbnail_4536' );
