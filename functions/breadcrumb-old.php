...
function breadcrumb( $args = array() ) {
	$defaults = array(
		'container'         => 'div',
		'container_class'   => 'clearfix padding-0-10px inner',
		...
		'home_text'         => 'ホーム',
		'delimiter'         => '<li><i class="fab fa-angle-right" aria-hidden="true"></i><i class="fab fa-folder-open-o" aria-hidden="true"></i></li>',
	...
	function current_crumb_tag( $current_permalink, $current_text = '', $args = array(), $current_class = 'current-crumb' ) {
		...
		$current_before     = '<li' . $current_class . $args->li_microdata . '><span' . $args->title_microdata . '>';
	...
	if (
		...
		if ( is_home() && ! is_front_page() ) {
			...
			if ( get_post_type() == 'post' ) {
				...
				// 記事のタイトル非表示 $breadcrumb_html .= current_crumb_tag( get_the_permalink( $single->ID ), get_the_title( $single->ID ), $current_microdata );
				...
				// カスタム投稿などもタイトル非表示 $breadcrumb_html .= current_crumb_tag( get_the_permalink( $single->ID ), get_the_title( $single->ID ), $current_microdata );
			}
